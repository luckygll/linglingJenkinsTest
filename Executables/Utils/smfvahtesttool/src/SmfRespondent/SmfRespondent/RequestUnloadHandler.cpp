...
RequestUnloadHandler::RequestUnloadHandler(smf::RequestServiceTransmitter* pTransmitter, const std::string& sName, unsigned long lIdentifier)
					:RequestHandler(pTransmitter,sName, lIdentifier)
{

}

RequestUnloadHandler::~RequestUnloadHandler()
{


}

void RequestUnloadHandler::Process(const smf::ConnectionIdentity id, smf::RequestServiceReceiver::Payload& payload)
{
	TiXmlDocument xmlStream;
	const char* pReturn = xmlStream.Parse(payload.GetPayload());
	if (pReturn != NULL)
	{
		xmlStream.SaveFile("RequestUnloadVAA.xml");
	}
