...
RequestDeactivateHandler::RequestDeactivateHandler(smf::RequestServiceTransmitter* pTransmitter, const std::string& sName, unsigned long lIdentifier)
					:RequestHandler(pTransmitter,sName, lIdentifier)
{

}

RequestDeactivateHandler::~RequestDeactivateHandler()
{


}

void RequestDeactivateHandler::Process(const smf::ConnectionIdentity id, smf::RequestServiceReceiver::Payload& payload)
{
	TiXmlDocument xmlStream;
	const char* pReturn = xmlStream.Parse(payload.GetPayload());
	if (pReturn != NULL)
	{
		xmlStream.SaveFile("RequestDeactivateVAA.xml");
	}
